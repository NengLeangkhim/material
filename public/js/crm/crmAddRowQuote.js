...
function checkBranchReminderRow(branId2) {  // function to check trash button on row quote branch item
    ...
    if (RowBranch.length === 0) {
        ...
        jQuery.each(RowBranch, function(index, val) {
            ...
            } else { // in this it mean it the same branch id
                RowBranch[index].push(branId2);
            }
        });
...
$(document).ready(function() {
    ...
    $(document).on('click', '#btnAddRowQuoteItem', function(event, issetBranId) {
        ...
        if (typeof(issetBranId) != 'undefined') {
            console.log('branch id isset=='+issetBranId);
            branId2 = issetBranId;
        }


        checkBranchReminderRow(branId2);
        ...
        var tblRow =
            ...
            '<button style="width: auto;" class="trashRemove_' + branId2 + ' btnRemoveRow btn btn-denger" id="' + i + '" data-code="' + branId2 + '" ><span><i style="color:#d42931;" class="fa fa-trash"></i></span></button>' +
            '</td>' +
            '</tr>';
        i++;
        j++;
        $('#add_row_tablequoteItem' + branId2 + '').append(tblRow);
        clearTrashOnRow(branId2,'add');
    });
    ...
    function remove_count_RowBranch(branId) {
        // console.log('count length row=' + RowBranch.length);
        var countArr = 0;
        if (RowBranch.length > 0) {
            jQuery.each(RowBranch, function(index, val) {
                if (val[0] == branId) {
    ...
    function clearTrashOnRow(branId,typeClick) {

        if(typeClick == 'remove'){
            var branchLenght = remove_count_RowBranch(branId);
            if (branchLenght <= 1) {
                $('.trashRemove_' + branId + '').attr("disabled", true);
            } else {
                $('.trashRemove_' + branId + '').attr("disabled", false);
            }
        }else{
            $('.trashRemove_' + branId + '').attr("disabled", false);
        }
