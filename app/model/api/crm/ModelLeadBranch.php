...
class ModelLeadBranch extends Model
{
    ...
    public static function GetLeadBranch($status){
        $sql = '';
        if($status=='all'){
            $sql = '';
        }else{
        ...
        where ld.status=true and ld.is_deleted=false $sql
		ORDER BY crm_lead.id,lb.id ASC
        ";
    }
    public static function getleadBranchDataTable($request,$status){
        $table = '('.self::GetLeadBranch($status).') as foo';
    ...
    //function get Lead Branch 
    public static function GetLeadBranchByAssign($status,$userid){
        $sql = '';
        if($status=='all'){
            $sql = '';
        }else{
        ...
        LEFT JOIN crm_lead_items clitem on clitem.crm_lead_branch_id = lb.id
        LEFT JOIN stock_product sp on sp.id= clitem.stock_product_id
        where ld.status=true and ld.is_deleted=false $sql and lb.create_by=$userid
		ORDER BY crm_lead.id,lb.id ASC
        ";
    }
}
