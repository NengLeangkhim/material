...
class ModelHrmManagerFollowUp extends Model
{
    ...
    public static function hrm_get_edit_manager_follow_up($id){
        $follow_up_user = DB::table('hr_performance_manager_follow_up as pfm')
                            ...
                            ->join('hr_performance_follow_up as pf','pfm.hr_performance_follow_up_id','=','pf.id')
                            ->join('hr_performance_schedule as ps','pf.hr_performance_schedule_id','=','ps.id')
                            ->join('ma_user_detail as sd','ps.create_by','=','sd.ma_user_id')
                            ->join('ma_user as s','ps.ma_user_id','=','s.id')
                            ->join('ma_user_detail as sdd','pfm.create_by','=','sdd.ma_user_id')
                            ->join('hr_performance_plan_detail as pd','ps.hr_performance_plan_detail_id','=','pd.id')
    ...
    public static function hrm_update_Manager_follow_up($id_manager,$userid,$follow_up_id,$percent,$score,$cmt,$status){
        return DB::select('SELECT public.update_hr_performance_manager_follow_up(?,?,?,?,?,?,?)',array($id_manager,$userid,$follow_up_id,$percent,$score,$cmt,$status));
    }
}
