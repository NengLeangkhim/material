    ...
    public function welcome(){
        if (session_status() == PHP_SESSION_NONE) {
            ...
            if(perms::check_perm()){
                ...
                $head=DB::select("select s.name,s.name_kh,s.id_number,s.image ,cdm.type,p.name as ma_position,s.contact
				...
				and cdm.type<>'normal'
				and s.status='t'");
                $uself=DB::select("select s.name,s.name_kh,s.id_number,s.image,p.name as ma_position,s.contact from ma_user s join ma_position p on p.id=s.ma_position_id where s.id=$user_id");
