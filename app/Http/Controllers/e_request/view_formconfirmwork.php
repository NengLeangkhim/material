...
class view_formconfirmwork extends Controller
{
    function formconfirmwork(){
        ...
        $q=DB::select("select s.name,p.name as position,d.name as dept,s.sex,s.id_number from staff s join position p on p.id=s.position_id join ma_company_dept d on d.id=s.ma_company_dept_id where s.id=$user_id");
