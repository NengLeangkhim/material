...
class check_perm{
    ...
    public function permi_get($id){
        ...
        if($r){
            ...
            if($r['type']=='top'){
                ...
                (select name from ma_user where id=erd.action_by) as action_by,
    ...
    public function get_view_val($s,$hr,$id){
        ...
        (select s.name from e_request_detail e join ma_user s on s.id=e.action_by where e.e_request_id=er.id ORDER BY e.id desc limit 1 offset 0) as action_by
        from e_request er
        join e_request_form_detail erfd on erfd.id=er.e_request_form_detail_id
        join e_request_form erf on erf.id=erfd.e_request_form_id
    ...
    public function get_report_val($s,$id,$f,$t){
        ...
        $sql="SELECT er.id,(select name from ma_user where id=er.create_by) as request_by,
