...
class view_requestform extends Controller
{
    function requestform(){
        ...
        if(isset($v0)){
            $q=DB::select("select s.name,p.name as position,d.name as dept from ma_user s join position p on p.id=s.position_id join ma_company_dept d on d.id=s.ma_company_dept_id where s.id=".$v0['to']);
        ...
        $q=DB::select("select s.name,p.name as position,d.name as dept from ma_user s join position p on p.id=s.position_id join ma_company_dept d on d.id=s.ma_company_dept_id where s.id=$user_id");
        $r=ere_get_assoc::assoc_($q)[0];
        if($r){
            $pos=$r['position'];
            $name=$r['name'];
