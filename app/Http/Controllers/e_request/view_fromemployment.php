...
class view_fromemployment extends Controller
{
    function fromemployment(){
        ...
        $q=DB::select("select id,name from ma_position where ma_group_id<>1 and ma_group_id<>6");
        $r=ere_get_assoc::assoc_($q);
        $pos=$r;

        $q=DB::select("select id,name from ma_company_dept where ma_company_id=8");
        $r=ere_get_assoc::assoc_($q);
        $dept=$r;
