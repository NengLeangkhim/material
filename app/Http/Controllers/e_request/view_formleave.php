...
class view_formleave extends Controller
{
    function formleave(){
        ...
        $q=DB::select("select s.name,s.id_number,p.name as position,d.name as dept from position p join staff s on s.position_id=p.id join ma_company_dept d on s.ma_company_dept_id=d.id where s.id=$user_id");
        ...
        where p.group_id <>1 and s.id_number is not null and s.ma_company_dept_id=(select ma_company_dept_id from staff where id=$user_id)
        order by name ");
        $r=ere_get_assoc::assoc_($q);
        $transfer_to=$r;
