...
class productCompany extends Controller
{
    ...
    public function getaddProductCompanyrequest(){
        ...
        if(perms::check_perm_module('STO_010201')){//module codes
            ...
            $company_id="(select cd.ma_company_id from staff s join ma_company_detail cd on cd.id=s.ma_company_detail_id where cd.status='t' and s.id=$st_id)";
            $r=array();
            $r[]='out';
            $r[]=DB::select("select cd.ma_company_id,cd.company from staff s join ma_company_detail cd on cd.id=s.ma_company_detail_id where cd.status='t' and s.id=$st_id");
    ...
    public function companyDashboard(){
        ...
        if(perms::check_perm_module('STO_0108')){//module codes
            $sql="SELECT p.id,p.name as name,
            (select sum(q.qty) from product_qty q join ma_company_detail cd on cd.id=q.company_detail_id where q.product_id=p.id and cd.ma_company_id=(select cd.ma_company_id from staff s join ma_company_detail cd on cd.id=s.ma_company_detail_id where s.id=".$_SESSION['userid']." )) as qty,
            (select sum(q.qty) from product_qty q join ma_company_detail cd on cd.id=q.company_detail_id where q.product_id=p.id and cd.ma_company_id=(select cd.ma_company_id from staff s join ma_company_detail cd on cd.id=s.ma_company_detail_id where s.id=".$_SESSION['userid']." ) and q.action_type='in') as import,
            (select sum(q.qty) from product_qty q join ma_company_detail cd on cd.id=q.company_detail_id where q.product_id=p.id and cd.ma_company_id=(select cd.ma_company_id from staff s join ma_company_detail cd on cd.id=s.ma_company_detail_id where s.id=".$_SESSION['userid']." ) and q.action_type='out') as request
            from product p
            ...
            where 't'
            and pc.ma_company_id=(select cd.ma_company_id from staff s join ma_company_detail cd on cd.id=s.ma_company_detail_id where s.id=".$_SESSION['userid'].")
            group by p.id having (select sum(q.qty) from product_qty q join ma_company_detail cd on cd.id=q.company_detail_id left join product p on p.id=q.product_id where q.product_id=p.id and cd.ma_company_id=(select cd.ma_company_id from staff s join ma_company_detail cd on cd.id=s.ma_company_detail_id where s.id=".$_SESSION['userid'].")) is not null";
            $com=DB::select($sql);
